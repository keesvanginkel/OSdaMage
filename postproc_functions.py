...
import time as time
import rasterio
#from main_functions import load_config script can end up in a loop if you do this
...
"""
FUNCTIONS FOR AGGREGATION OF LISFLOOD GRID APPROACH
"""

#CORE MODEL? NO
def agg_damage_rasters(landuse_codes,landuse_raster,damage_raster,region_geoms,nanvalue):
    """Function to aggregate damage in a certain region over land use classes defined in a land use raster.
    
    Arguments:
        *landuse_codes* (list) : list containing all the values that can exist in the land use raster
        *landuse_raster* (string) : path to the land use raster, which should be a .tif file
        *damage_raster* (string) : path to the damage raster, which should be a .tif file
        *region_geoms* (GEJSON polygon geometry) : the geometry describing the area over which to aggregate
        *nanvalue* (float) : the value used in the raster to depict nan-values - choosing the wrong value will completely mess up the results
     
    Returns:
        *results* (2D numpy array) : first column: land_use_codes; second column: the aggregated damage  
    
    """
    #Open a part of the damage grid
    with rasterio.open(damage_raster) as src:
         out_image, out_transform = mask(src, region_geoms, crop=True) #the projection of the raster is EPSG 3035 
    damage_raster = out_image[0] #out_image is a numpy masked array
    #damage_raster[np.isin(damage_raster,-9999.0)]=np.nan #replace the raster-no data value with a numpy nan; -3.4028234663852886e+38
    damage_raster[np.isin(damage_raster,nanvalue)] = np.nan #replace the raster-no data value with a numpy nan
    
    #Open a part of the land use map (Corine-2012 grid)
    with rasterio.open(landuse_raster) as src:
         out_image, out_transform = mask(src, region_geoms, crop=True) #the projection of the raster is EPSG 3035 
    landuse_raster = out_image[0] #out_image is a numpy masked array
    
    results = np.zeros((len(landuse_codes),2))
    
    for index, code in enumerate(landuse_codes):
        mask2 = np.isin(landuse_raster, code) #create a mask with trues for the right land use class
        #print("For code: {}; the damage is: {} Euros".format(code,np.nansum(damage_raster[mask])))      
        #corine_class["Damage"][code-1] = np.nansum(damage_raster[mask]) #UGLY FIX for the indexing; fix later
        results[index] = (code,np.nansum(damage_raster[mask2]))
    return results

#CORE MODEL? NO
def CLCtoGRID(corine_class,CLC):
    "Converts CLC code to grid code"
    row = corine_class.loc[corine_class['CLC_CODE'] == CLC] #gives the right row
    grid = row.values[0][0] #gives the corresponding grid code
    return grid

#### AGGREGATE CORINE LANDCOVER MAPS ####

#CORE MODEL? NO
def aggregate_Corine(JRC_results,level,corine_class):
    """Function to aggregate DataFrame containing damages according to Corine land_use classses to a certain degree
    
    Arguments:
        *JRC_results* (Pandas Dataframe) -- DataFrame with regions as rows; and damages per land_use class in the columns
        *level* (Integer) -- 
                if level = 2 e.g. add Continuous urban fabric (111) and Discontinuous urban fabric (112) to Urban fabric (11X)
                if level = 1 aggregate another level
                if level = 0 calculate total
        *corine_class (Pandas Dataframe) -- Containing the Corine Landcover classification read from XLS file
        
    Returns:
        *JRC_results_agg* (Pandas Dataframe) -- Similar to input dataframe, but with aggregated columns
    """
    
    dict = {11: [111,112], #dict for aggregating from level 3 to level 2
        12: [121,122,123,124],
        13: [131,132,133],
        14: [141,142],
        21: [211,212,213],
        ...
        51: [511,512],
        52: [521,522,523],
        99: [999,990,995] #mind: there are two 999 grid values: 48 (no data) and 255 (unclassified)
       }

    dict2 = {1: [11,12,13,14], #dict for aggregating from level 2 to level 1
             2: [21,22,23,24],
             3: [31,32,33],
             4: [41,42],
             5: [51,52],
    ...
    dict3 = {'total': [1,2,3,4,5,9]}
  
    JRC_results_agg = pd.DataFrame(index=JRC_results.index)
    JRC_results_agg2 = pd.DataFrame(index=JRC_results.index)
    JRC_results_agg1 = pd.DataFrame(index=JRC_results.index)    
         
    for key in dict:
        CLCs = dict[key]
        grid_codes = [CLCtoGRID(corine_class,x) for x in CLCs]
        df = JRC_results[grid_codes] #new dataframe with only the columns to be summed
        JRC_results_agg2[key]=df.sum(axis=1)

    if level == 2:
        JRC_results_agg = JRC_results_agg2
        
    else:    
        for key in dict2:
            df = JRC_results_agg2[dict2[key]]
            JRC_results_agg1[key]=df.sum(axis=1)
            
        if level == 1:
            JRC_results_agg = JRC_results_agg1
        else:     
            for key in dict3:
                df = JRC_results_agg1[dict3[key]]
                JRC_results_agg[key]=df.sum(axis=1)
        
    return (JRC_results_agg)

#CORE MODEL? NO
def label2(CLC,corine_class):
    if CLC < 99:
        CLC = int(str(CLC)+str(1))
        row = corine_class.loc[corine_class['CLC_CODE'] == CLC] #gives the right row
        label = row.values[0][3] #gives the corresponding grid code
    else:
        label = 'Unclassified'
    return label

#CORE MODEL? NO
def label3(CLC,corine_class):
    if CLC > 100:
        row = corine_class.loc[corine_class['CLC_CODE'] == CLC] #gives the right row ####DIT kan nog mooier; ook nog samenvoegen met andere functie!
        label = row.values[0][4] #gives the corresponding grid code
    else:
        label = 'ERROR'
    return label

#### AGGREGATE LUISA LANDCOVER MAPS ####
#CORE MODEL? NO
def aggregate_Luisa(JRC_results,level,landuse_codes):
    """Function to aggregate DataFrame containing damages according to Luisa land_use classses to a certain degree
    
    Arguments:
        *JRC_results* (Pandas Dataframe) -- DataFrame with regions as rows; and damages per land_use class in the columns; e.g. 1111 as column header
        *level* (Integer) -- 
                if level = 4 do nothing
                if level = 3 aggregate groups like Urban fabric medium dens. (1121) and Urban fabric low dens. (1122) etc.
                if level = 2 e.g. add Continuous urban fabric (111) and Discontinuous urban fabric (112) to Urban fabric (11X)
                if level = 1 aggregate another level
                if level = 0 calculate total
        *luisa_class* (Pandas Dataframe) -- Containing the Luisa Landcover classification read from XLS file
    ...
    JRC_results_agg_3 = pd.DataFrame(index=JRC_results.index)
    JRC_results_agg_2 = pd.DataFrame(index=JRC_results.index)
    JRC_results_agg_1 = pd.DataFrame(index=JRC_results.index)
    JRC_results_agg_0 = pd.DataFrame(index=JRC_results.index)

    #create the dicts based on the landuse_codes list
    dict43 = dict.fromkeys(set([str(i)[:-1] for i in landuse_codes]), 0)
    for key in dict43: #e.g. key 112, values [1121, 1122, 1123]
        dict43[key] = [i for i in landuse_codes if str(i)[:-1] == key] 
    dict32 = dict.fromkeys(set([i[:-1] for i in list(dict43.keys())]))
    for key in dict32: #e.g. key 11, values [111, 112]
        dict32[key] = [i for i in list(dict43.keys()) if i[:-1] == key]
    dict21 = dict.fromkeys(set([i[:-1] for i in list(dict32.keys())]))
    for key in dict21: #e.g. key 1, values, 11, 12, 13, 14
        dict21[key] = [i for i in list(dict32.keys()) if i[:-1] == key]
    dict10 = {'total' : list(dict21.keys())} # key = total, values = [0:6]

    #to the actual summation
    for key in dict43:
        JRC_results_agg_3[key] = JRC_results.loc[:,dict43[key]].sum(axis=1)
    for key in dict32:
        JRC_results_agg_2[key] = JRC_results_agg_3.loc[:,dict32[key]].sum(axis=1)
    for key in dict21:
        JRC_results_agg_1[key] = JRC_results_agg_2.loc[:,dict21[key]].sum(axis=1)
    for key in dict10:
        JRC_results_agg_0[key] = JRC_results_agg_1.loc[:,dict10[key]].sum(axis=1)

    #determine which dataframe to return
    if level == 4:
        output = JRC_results #return input value; no aggregation
    elif level == 3:
        output = JRC_results_agg_3
    elif level == 2:
        output = JRC_results_agg_2 
    elif level == 1:
        output = JRC_results_agg_1 
    elif level == 0:
        output = JRC_results_agg_0 
    else:
        raise ValueError('Invalid aggregation level: \'{}\'. Should be 4 (no aggregation), 3, 2, 1 or 0 (total).'.format(level))
    
    return output.reindex(sorted(output.columns), axis=1)

#CORE MODEL? NO
def label_luisa2(df,luisa_class):
    """
    Determines the land use labels for the Luisa codes in the df. Also works for Panda Series.
    
    Arguments:
    
        *df* (Pandas Dataframe) -- Rows: regions; columns: LUISA land use codes (integers e.g. 1213, 123 etc.)
        *luisa_class* (Pandas Dataframe) -- pandas dataframe created from teh excel file: Luisa_Corine_legend.xls
        
    Returns:
        *luisa_class* (List) -- list with the column names corresponding to the LUISA codes
    
    
    TODO: remove the old functions.
    """
    if isinstance(df, pd.DataFrame):
        cats = df.columns.values #landuse categories
    elif isinstance(df,pd.Series):
        cats = df.index
    
    level = len(str(cats[0])) #determine the aggregation level of the df containing Luisa data
    labels = []
    if level == 4:
        col = 'luisa2label'
        labels = [luisa_class.loc[luisa_class['LUISA_GlobCov_code'] == i,col].values[0] for i in cats]
    elif level == 3:
        col = 'CLC_LABEL3_GlobCov'
        for i, cat in enumerate(cats):
            row = luisa_class[:-1].LUISA_GlobCov_code.apply(lambda x: int(str(int(x))[:-1])) == int(cat)
            label = luisa_class[:-1].loc[row,col].values[0]
            labels.append(label)
    elif level == 2:
        col = 'CLC_LABEL2'
        for i, cat in enumerate(cats):
            row = luisa_class[:-1].LUISA_GlobCov_code.apply(lambda x: int(str(int(x))[:-2])) == int(cat)
            label = luisa_class[:-1].loc[row,col].values[0]
            labels.append(label)
    elif level == 1:
        col = 'CLC_LABEL1'
        for i, cat in enumerate(cats):
            row = luisa_class[:-1].LUISA_GlobCov_code.apply(lambda x: int(str(int(x))[:-3])) == int(cat)
            label = luisa_class[:-1].loc[row,col].values[0]
            labels.append(label)
    elif level == 0:
        labels.append('total')
...
#CORE MODEL? NO
def region_raster_risk_LUISA_all_lu(regionname):
    """
    For a given region (NUTS-3), calculates the flood risk (EAD) for each land use type.
    
    Arguments:
        *regionname* (string) - NUTS-3 name of the region
    
    Returns:
        *results* (Panda series) - indices = land use codes; values = flood risk
    """
    
    if os.path.exists("..\\luisa_temp\\{}_EAD_LUISA_all.pkl".format(regionname)): 
        print('{} already finished!'.format(regionname))
        return None
    
    print("LUISA ALL calculation started for: {}".format(regionname))
    
    try:
        ### SOME PARAMETERS 
        nanvalue = -3.4028234663852886e+38

        #### SET ALL THE PATHS
        main_folder = "D:\\Europe_trade_disruptions"

        #damage_raster = "D:\\ginkel\\Europe_trade_disruptions\\JRC_damages\\Danube\\calibration_2018_v2_inf\\damageMaps_RP200\\mergeMaps\\maxDamage_Danube.tif"
        landuse_raster_path = os.path.join(main_folder,"JRC_landuse\\LUISA_base_map_v2\\LUISA_basemap_2_0.tif")
        infra_rasters = os.path.join(main_folder,"JRC_damages\\2019_04_01_LUISA_infraonly")
        all_rasters = os.path.join(main_folder,"JRC_damages\\2019_04_01_LUISA_allclasses")

        #open flood protection level:
        fpl_path = load_config()['paths']['fpl_raster']

        #Open Corine Land Cover 2012 classification
        landcoverlegend_path = os.path.join(main_folder,"JRC_landuse\\Luisa_Corine_legend.xls")
        luisa_class = pd.read_excel(landcoverlegend_path)
        landuse_codes = list(map(int,list(luisa_class.LUISA_GlobCov_code)[:-1])) #convert to integers; drop the last value (NaN category) #ugly fix

        #Open the shapefile containing the spatial regions; and reduce to regions relevant for Europe
        NUTS_shape = os.path.join(main_folder,"NUTS-2_shapes\\NUTS_2016\\NUTS_RG_01M_2016_3035_LEVL_3.shp")
        NUTS_poly = gpd.read_file(NUTS_shape)
        remote_regions = ["ES703","ES704","ES705","ES706","ES707","ES708","ES709",
                          "FRY10","FRY20","FRY30","FRY40","FRY50",
                          "PT300","PT200"]
        regions = NUTS_poly[~NUTS_poly['NUTS_ID'].isin(remote_regions)]

        region = regions.loc[regions['NUTS_ID'] == regionname] 
        region_geoms = region.geometry.values[0] #only take the shapely polygon
        region_geoms = [mapping(region_geoms)] #convert shapely geometry to GeoJSON format

        ### READ EVENTS AS 3D NUMPY ARRAY
        events = [10,20,50,100,200,500]
        events.reverse() #start with the largest return period
        #issue: rp10 has a different nan-value
        damage_rasters = [0] * len(events)
        for i, event in enumerate(events):
            raster_path = os.path.join(all_rasters,"damage_EU_rp{}y.tif".format(event))
            with rasterio.open(raster_path) as src:
                out_image,out_transform = mask(src,region_geoms,crop=True)
            damage_raster = out_image[0]
            if event != 10: #the rp10 has just zeros instead of the nan-value
                damage_raster[np.isin(damage_raster,nanvalue)] = 0 #replace the nan-value with a 0.  
            #print(damage_raster.shape) #can do: check if all the rasters have the same size.
            damage_rasters[i] = damage_raster
        damage_rasters = np.stack(damage_rasters)
        #now, damage raster is a 3D-numpy array; containing the 6 RPs (starting with 500) * L * B

        ### READ FP LEVEL
        with rasterio.open(fpl_path) as src:
            out_image,out_transform = mask(src,region_geoms,crop=True)
            fpl_raster = out_image[0]
            fpl_raster[np.isin(fpl_raster,nanvalue)] = np.NaN
        #print(fpl_raster.shape)
        
        #dirty fix for bug (21-11-2019) The bug is caused by the fact that the np mask function does not give the array size for the different rasters
        if damage_rasters.shape[1:3] == fpl_raster.shape:
            print('{}: Same dimensions, no problem'.format(regionname))
        else:
            print('{}: Dimensions do not match: damage_rasters: {} and fpl_rasters: {}; we assumed FPL = {} everywhere'.format(regionname,damage_rasters.shape,fpl_raster.shape,np.nanmean(fpl_raster)))
            fpl_raster = np.full(damage_rasters.shape[1:3],np.nanmean(fpl_raster)) 
            #create a new raster with the right dimension, filled with the mean FPL of the original raster
            
        ### Calculates the actual flood risk in the region
        size = fpl_raster.shape
        EAD = np.empty(size)
        for index in np.ndindex(size):
            dam = [damage_rasters[i,index[0],index[1]] for i in [0,1,2,3,4,5]]
            fpl = fpl_raster[index] #get the right Flood protection level for this index
            if np.isnan(fpl) or np.count_nonzero(dam) == 0:
                risk = np.NaN #outside the mask boundary
            else: #do the actual calculation
                risk = risk_FP(dam,events,fpl)
                #print(risk)
            EAD[index[0],index[1]] = risk

        ### SUM PER LANDUSE TYPE
        #Open a part of the land use map (Corine-2012 grid)
        with rasterio.open(landuse_raster_path) as src:
            out_image, out_transform = mask(src, region_geoms, crop=True) #the projection of the raster is EPSG 3035 
        landuse_raster = out_image[0] #out_image is a numpy masked array

        results = pd.Series(index=landuse_codes,name=regionname)

        for index, code in enumerate(landuse_codes):
            mask2 = np.isin(landuse_raster, code) #create a mask with trues for the right land use class
            results[code] = np.nansum(EAD[mask2])

        results.to_pickle("..\\luisa_temp\\{}_EAD_LUISA_all.pkl".format(regionname))
...
def region_raster_risk_LUISA_infra_lu(regionname):
    """
    For a given region (NUTS-3), calculates the flood risk (EAD) for each land use type.
    ...
    Returns:
        *results* (Panda series) - indices = land use codes; values = flood risk
    """
    if os.path.exists("..\\luisa_temp\\{}_EAD_LUISA_infra.pkl".format(regionname)): 
        print('{} already finished!'.format(regionname))
        return None
    
    print("LUISA infra calculation started for: {}".format(regionname))
    ...
    try:   
        ...
        damage_rasters = [0] * len(events)
        for i, event in enumerate(events):
            raster_path = os.path.join(infra_rasters,"damageInf_EU_rp{}y.tif".format(event))
            with rasterio.open(raster_path) as src:
                out_image,out_transform = mask(src,region_geoms,crop=True)
            damage_raster = out_image[0]
            #if event != 10: #the rp10 has just zeros instead of the nan-value - not necessary for the infra_rasters
            damage_raster[np.isin(damage_raster,nanvalue)] = 0 #replace the nan-value with a 0.  
            #print(damage_raster.shape) #can do: check if all the rasters have the same size.
            damage_rasters[i] = damage_raster
        damage_rasters = np.stack(damage_rasters)
        ...
        ### READ FP LEVEL
        with rasterio.open(fpl_path) as src:
            out_image,out_transform = mask(src,region_geoms,crop=True) #bug: gives a 132*186 raster
            fpl_raster = out_image[0] #bug: gives a 132*186 raster
            fpl_raster[np.isin(fpl_raster,nanvalue)] = np.NaN
        ...
        results.to_pickle("..\\luisa_temp\\{}_EAD_LUISA_infra.pkl".format(regionname))
...
### todo: create remote regions list with a function
#Can replace many lines of some other functions (suspects/remote) in this script

def region_raster_risk_CORINE_infra_lu(regionname):
    """
    For a given region (NUTS-3), calculates the flood risk (EAD) for each land use type.
    
    
    <TODO: figure out which CORINE classification is actually used!!!>
    ...
    try:
        ...
        #damage_raster = "D:\\ginkel\\Europe_trade_disruptions\\JRC_damages\\Danube\\calibration_2018_v2_inf\\damageMaps_RP200\\mergeMaps\\maxDamage_Danube.tif"
        landuse_raster_path = os.path.join(main_folder,"JRC_landuse\\Corine_2012_g100_clc12_V18_5a\\g100_clc12_V18_5.tif")
        infra_rasters = os.path.join(main_folder,"JRC_damages\\2019_10_07_CORINE_infraonly")
        ...
        #Open Corine Land Cover 2012 classification
        landcoverlegend_path = os.path.join(main_folder,"JRC_landuse\\Luisa_Corine_legend.xls")
        #landuse_codes = [111, 112, 121, 122, 123,
        #          124,131,132,133,141,142,211,212,213,221,222,223,231,241,242,243,
        #           244,311,312,313,321,322,323,324,331,332,333,334,335,411,412,421,
        #           422,423,511,512,521,522,523,45,990]
        landuse_codes = list(range(1,45)) #reconstruct the list of CORINE codes
        landuse_codes.extend([255,46])
        ...
        for i, event in enumerate(events):
            raster_path = os.path.join(infra_rasters,"CLC2012_inf_rp{}.tif".format(event))
            with rasterio.open(raster_path) as src:
                out_image,out_transform = mask(src,region_geoms,crop=True)
            damage_raster = out_image[0]
            #if event != 10: #the rp10 has just zeros instead of the nan-value - not necessary for the infra_rasters
        ...
        for index, code in enumerate(landuse_codes):
            mask2 = np.isin(landuse_raster, code) #create a mask with trues for the right land use class
            results[code] = np.nansum(EAD[mask2])
        results.to_pickle("..\\corine_temp\\{}_EAD_CORINE_infra.pkl".format(regionname))
...
def NUTS_0_list_old():
    ...
    Returns a list with NUTS-0 codes
    
    """
    NUTS_0_list = ["AL", # Albania                NOT EU - Candidate country
          "AT", # Austria
          "BE", # Belgium
          "BG", # Bulgaria
          "CH", # Switzerland            NOT EU! - EFTA country
          "CY", # Cyprus
          "CZ", # Czech Republic
          "DE", # Germany
          "DK", # Denmark
          ...
          "HU", # Hungary
          "IE", # Ireland
          "IS", # Iceland                #NOT EU - EFTA country
          "IT", # Italy
          "LT", # Lithuania
          "LU", # Luxembourg
          "LV", # Latvia
          "LI", # Liechtenstein          #NOT EU - EFTA country
          "MT", # Missing: MT - Malta
          "ME", # Montenegro             #NOT EU - Candidate country
          "MK", # North-Macedonia        #NOT EU - Candidate country
          "NL", # Netherlands
          "NO", # Norway                 #NOT EU - EFTA country
          "PL", # Poland
          "PT", # Portugal
          "RO", # Romania
          "RS", # Serbia                 #NOT EU - Candidate country
          "SE", # Sweden
          "SI", # Slovenia
          "SK", # Slovakia
          "TR", # Turkey                 #NOT EU - Candidate country
          "UK" # United Kingdom
         ]
    return NUTS_0_list
...
def NUTS_3_remote(**kwargs):
    ...
    Optional keyword arguments (boolean):
            ...
            "Spain"       : Melilla (Spanish North Coast) (defautl True)
...
def SSP_correction():
    """
    Calculates GDP correction factors for different SSP scenarios.
    
    Input: no
    
    Returns:
        *da* (xarray DataSet): with 4 coordinates/dimensions:
            1) Model (i.e. the source of the projection)
            2) Scenario (SSP1 ... SSP5)
            3) Region (country names - 3 letter codes)
            4) Year (2010 - 2100)
            
            And 2 variables:
            1) GDP (projected GDP (PPP) per capita)
            2) GDP_rel (idem, but relative to the GDP in 2015)
    """
    
    #OPEN EXCEL FILE FROM INPUT_DATA AS A DATAFRAME
    input_data = load_config()['paths']['input_data']
    filename = load_config()['filenames']['SSP_corr'] 
    file = os.path.join(input_data,filename)
    df = pd.read_excel(file,index_col=[0,1,2],nrows=1796) #['Model','Scenario','Region']
    df.drop(['Variable', 'Unit', 'Notes'],axis=1,inplace=True) #drop some unused columns and data
    df = df.stack() #Add the columns as a new dimension in the dataframe (from wide table to long table)

    #CONVERT DATAFRAME TO XARRAY DATASET
    da = df.to_xarray()
    da = da.rename({'level_3' : 'Year'})
    da = da.rename('GDP')
    da = da.drop(['Model'], dim='Model') #drop unused data
    da = da.drop(['Scenario (History)'], dim='Scenario') #drop unused data
    EU28 = EU28_3l()
    da = da.sel(Region = EU28) #Filter at only EU-28 member states
    da = da.to_dataset()

    #NORMALIZE GDP WITH RESPECT TO REFERENCE YEAR 2015
    da['GDP_rel'] = da.GDP/da.sel(Year=2015).GDP

    #selecting examples
    #da.sel(Model='OECD Env-Growth',Scenario='SSP1',Region=EU28, Year=[2015,2050,2080])
    #da.sel(Model='OECD Env-Growth',Scenario='SSP1',Region=EU28, Year=[2015])
...
### OLD FUNCTION, COULD BE HANDY LATER; not sure if it works completely fine now.
def aggregate_OSM_regions_csv(regions,OSM_result_dir):
    """
    Aggregates the OSM-results for a number of regions and returns as a DataFrame

    Arguments:
        *regions* (list of strings) -- List containing the names of the NUTS-3 regions over which to aggregate
        *OSM_result_dir* (string) -- Directory containing the model results (should have .csv files / or .pkls)
    ...
    file_paths = [] #list to save the usuable filepaths
    for region in regions:
        if not os.path.exists(os.path.join(OSM_result_dir,"{}.csv".format(region))):
            missing_files.append(region) #add to missing region list
        else:
            file_paths.append(os.path.join(OSM_result_dir,"{}.csv".format(region))) #append to the list of csv to be opened

    ini = pd.read_csv(file_paths[0]) #open the first csv to initialize the loop
    dam_cols = [s for s in ini.columns.values if 'dam' in s] #select the columns containing damage estimates    
    ...
    for path in tqdm(file_paths):
        #print(path)
        try:
            data = pd.read_csv(path,converters=conv)
            data_agg = au.aggregate_tuplecollection(data,dam_cols,"NUTS-3")
            OSM_outcomes=pd.concat([OSM_outcomes,data_agg],sort=False)
        except Exception as e:
            exceptions.append("This did not work for: {}, because of {}".format(path,e)) #most of them seem to be just empty csv files
